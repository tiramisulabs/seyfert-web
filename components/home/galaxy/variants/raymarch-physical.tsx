...
const marchFrag = /* glsl */ `
  ...
  #define STEPS     110     // integration steps (must cover grazing orbits)
  #define DT_BASE   0.105   // base proper step
  #define DT_GROW   0.030   // step grows per unit r beyond ~3 (adaptive)
  #define MAX_CROSS 3       // equatorial crossings accumulated per ray

  #define CAM_DIST  9.5     // camera distance from the hole
  #define CAM_ELEV  0.17    // camera elevation above disk plane (~10 deg)
  #define FOV       0.60    // half-angle field-of-view scale

  #define KEPLER    0.95    // Keplerian speed scale (fraction-of-c-ish)
  #define BEAM_PWR  3.2     // relativistic beaming exponent (Doppler^pwr)
  #define DISK_GAIN 1.55    // overall disk emission gain
  #define GLOW_GAIN 0.030   // volumetric near-plane glow gain
  ...
  vec3 diskEmission(float rad, float az, vec3 rayDir) {
    ...
    float inner = smoothstep(0.0, 0.06, t);           // razor ISCO lip ramp-in
    float outer = 1.0 - smoothstep(0.72, 1.0, t);     // outer fray
    float radial = inner * outer;
    radial *= 1.0 / (0.35 + 2.4 * t * t);             // steep outward falloff

    // KEPLERIAN differential rotation: inner gas winds faster (Ω ∝ r^-1.5)
    float omega = pow(R_ISCO / max(rad, R_ISCO), 1.5);
    float spin  = uTime * 0.55 * omega;

    // multi-scale advected filaments in (azimuth, radius)
    float a2 = az + spin;
    vec2 q = vec2(a2 * 2.6, t * 6.0);
    float wx = fbm(q * 0.8 + 3.1) - 0.5;              // domain warp → stringy
    vec2 fq = vec2(q.x + wx * 1.4, q.y * 1.5);
    float broad = fbm(fq);
    float fine  = fbm(fq * vec2(2.4, 1.3) + 17.0);
    float fil = mix(broad, fine, 0.45);
    fil = pow(clamp(fil * 1.25, 0.0, 1.0), 1.5);
    float cn = fbm(vec2(a2 * 1.4 + 4.0, t * 5.0 - uTime * 0.05));
    float clump = smoothstep(0.66, 0.85, cn);         // hot orbiting clumps
    float emis = (0.45 + 0.95 * fil) + clump * 1.8;

    // temperature ramp: white-amber inner → amber → copper outer
    vec3 col = mix(HOT, WARM, smoothstep(0.0, 0.45, t));
    col = mix(col, COPPER, smoothstep(0.45, 1.0, t));

    // ── RELATIVISTIC DOPPLER (beaming + color) ──
    // disk tangential velocity (CCW) at this azimuth; equatorial plane = xz.
    float v = KEPLER * sqrt(R_ISCO / max(rad, R_ISCO)) * 0.74;
    vec2 ca = vec2(cos(az), sin(az));
    vec3 vel = vec3(-ca.y, 0.0, ca.x) * v;            // tangential, xz-plane
    float mu = dot(normalize(-rayDir), vel);          // approaching > 0
    float beta = clamp(v, 0.0, 0.95);
    float gamma = 1.0 / sqrt(max(1.0 - beta * beta, 1e-3));
    float doppler = 1.0 / (gamma * (1.0 - clamp(mu, -0.95, 0.95)) + 1e-3);
    float beam = pow(clamp(doppler, 0.25, 4.0), BEAM_PWR);

    float approach = clamp((doppler - 1.0) * 1.4, -1.0, 1.0);
    col = mix(col, HOT, max(approach, 0.0) * 0.55);   // approaching whitens
    col = mix(col, COPPER, max(-approach, 0.0) * 0.45); // receding reddens
  ...
  void main() {
    ...
    vec3 accum = vec3(0.0);        // accumulated emission (linear)
    ...
    for (int i = 0; i < STEPS; i++) {
      ...
      if (rad2d > R_ISCO * 0.9 && rad2d < R_OUT * 1.1) {
        ...
        float reach = smoothstep(R_OUT * 1.1, R_ISCO * 0.9, rad2d);
        vec3 g = mix(WARM, COPPER, smoothstep(R_ISCO, R_OUT, rad2d));
        accum += g * planeProx * reach * GLOW_GAIN * transmit * dt;
      }
    ...
    if (!captured) {
      col += sampleSky(normalize(vel)) * transmit;
    }

    // mark the shadow: captured rays render true black, opaque (the only place
    // alpha = 1). Pass 2 keeps it opaque black.
    float shadow = captured ? 1.0 : 0.0;

    float lum = max(col.r, max(col.g, col.b));
    float a = clamp(lum * 1.2, 0.0, 1.0);
    a = max(a, shadow * uIntro);          // shadow opaque after intro settles
    col *= (1.0 - shadow);                // shadow stays true black
...
const compositeFrag = /* glsl */ `
  ...
  void main() {
    ...
    float split = pow(edge, 1.3) * 0.0030 * (1.0 - smoothstep(0.12, 0.40, rh));
...
export default function GalaxyCanvas() {
  ...
  useEffect(() => {
    ...
    const resize = () => {
      ...
      marchProgram.uniforms.uScale.value = aspect > 1 ? 1 : 0.62;
