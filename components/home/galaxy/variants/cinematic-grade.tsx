...
//     azimuth is pre-twisted by an offset ∝ uTime / r^1.5 (Keplerian shear),
//     so inner radii wind faster — filaments visibly stretch and wrap over
//     ~30s instead of rigidly scrolling.
...
//     the azimuth so the silhouette undulates instead of being a clean ribbon.
...
//
// ── CINEMATIC GRADE pass (layered on top, this revision) ──────────────────
// Everything above accumulates scene-referred radiance into col. The block
// below the \"CINEMATIC GRADE\" marker is a display-referred colorist pipeline
// run once, like a DI timeline:
//   • ACES-fit filmic tone curve (Narkowicz) replaces the reinhard divide —
//     protects warm highlights, gives the movie-still shoulder.
//   • lifted-black toe: shadows settle onto deep #05070d space instead of
//     crushing to pure black (pure 0,0,0 reserved for the horizon).
//   • edge-aware chromatic aberration: radial RGB split that ramps with the
//     local edge energy (photon ring + hotspot) and radius — ~0 over empty
//     text-adjacent space, so no fringing where copy will sit.
//   • temperature gradient: disk whiter at the inner rim, ember-copper at the
//     limb; background stars get per-star color-temperature jitter.
//   • re-tuned additive bloom shoulders so the ring reads razor-sharp WITH a
//     believable wide halo.
//   • fine animated hash grain (luminance-weighted, ~1.5%) + a vignette
//     anchored to the HOLE position (uCenter), not the screen center.
//
  ...
  void main() {
    ...
    // ===== 1. background starfield (behind everything) =====
    {
      ...
      if (h > STAR_DENSITY) {
        ...
        // ── per-star color temperature jitter: each star drawn from a per-cell
        //    temp so the field reads like a real exposure (cool blue-white →
        //    neutral → warm amber), never a uniform tint.
    ...
    // We build a sheared azimuth: every radius orbits at WIND_TIME, but inner
    // radii get an EXTRA twist ∝ 1/r^1.5 so filaments wind & stretch over time.
    // swirlU is the azimuthal sample coordinate; sampling FBM along it makes
    // streaks that physically wrap around the hole.
    ...
    // Computed once, reused by the front band + inner lip. Gated by diskNear so
    // distant pixels effectively skip these octaves (the compiler still runs
    // them, but they only matter where diskNear > 0; perf stays bounded because
    // the values are multiplied out). Total heavy noise ≈ 6 octaves worth.
    float filaments = 0.5;
    float clumps    = 0.0;
    if (diskNear > 0.002) {
      // warp the azimuth/radius slightly with a coarse noise → stringy, organic
    ...
    // key off high-contrast structures (ring, hotspot) so empty text-adjacent
    // space gets zero fringing. Accumulated by the bright features below.
    float edgeKey = 0.0;
    ...
    // ===== 5. front accretion band — LIVING PLASMA =====
    {
      ...
      // limb skews deeper amber→copper. side ∈ [0..1], 1 = approaching.
    ...
    // Capture the graded luminance BEFORE the toe lift. This drives the
    // premultiplied alpha so genuinely empty space stays transparent over the
    // DOM — the toe color must not make the whole canvas faintly opaque.
    float sceneLum = max(col.r, max(col.g, col.b));

    // ── lifted-black toe ──
    // Settle shadows onto deep #05070d space instead of crushing to 0,0,0.
    // Gated by sceneLum AND by where there's any nearby radiance, so the lift
    // tints the composition's shadows without flooding empty page space.
    // Luminance-weighted; pure black reserved for the event horizon.
    float shadowW = 1.0 - smoothstep(0.0, 0.10, sceneLum);
    // proximity to the system: lift only near the disk/ring, fade by radius
    float nearSystem = smoothstep(1.7, HOLE_R, r);
    col = mix(col, max(col, TOE), shadowW * nearSystem);
    ...
    // Driven in screen space so it never swims with parallax.
    {
      ...
      col += g * GRAIN_AMOUNT * gw;
      col = max(col, vec3(0.0));
    }

    // ── premultiplied output ──
    // Alpha follows the PRE-TOE scene luminance so empty space stays fully
    // transparent (the toe lift + grain must not make the canvas opaque over
    // DOM text)... EXCEPT the event horizon, which must occlude the page →
    // force alpha to 1 (black).
