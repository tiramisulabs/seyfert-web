...
<Callout title="Disabling the Cache">
...
A `custom resource` is just a new cache entity, so integrating it is relatively simple. Let's take the example of the [Cooldown](https://github.com/tiramisulabs/extra/blob/main/packages/cooldown/src/resource.ts) resource from the [cooldown](/guide/recipes/cooldown) package.
