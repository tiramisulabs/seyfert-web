...
Plugins can emit custom events with `api.events.emit(...)`. Gateway event names cannot be emitted from plugins.
...
Plugin option fragments are composed before final user options. The app can still override its own final configuration.
