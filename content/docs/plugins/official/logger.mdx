...
| `renderer` | `LoggerAdapter` | Primary output adapter. Defaults to `prettyRenderer()` and conventionally owns terminal output. |
| `transports` | `readonly LoggerAdapter[]` | Additional adapters invoked for every entry. Choose non-printing sinks when the renderer already owns the terminal. |
...
<Callout type="info">
`useLogger()` throws if the plugin hasn't been set up. If multiple clients own different logger instances, bare `useLogger()` also throws outside an interaction scope; pass the owning client as `useLogger(client)` instead. Outside a scope each call returns a *fresh* event, so capture it in a variable (as above) when you intend to `add()` then `emit()`.
</Callout>
...
| `evlogTransport(config?)` | With config, sends records through evlog drains with `silent: true`. Without config, it reuses the existing global evlog setup unchanged. |
| `pinoAdapter(instance)` | Wraps a Pino instance for either role, depending on its destination. |

evlog is an optional peer dependency and is imported only by its factories. `pinoAdapter()` accepts a structural `PinoLoggerLike`, so this package does not depend on Pino; install `pino` only when you use it.

<Callout type="warn">
**Redaction belongs to the sink.** `prettyRenderer()` does **not** redact. Configure redaction in your runtime/collector, in your Pino instance, or in evlog's config. evlog's built-in patterns (`creditCard`, `email`, `jwt`, …) do **not** cover Discord bot tokens — add a pattern for those.
</Callout>
...
Install Pino and wrap your own instance with `pinoAdapter`, so any Pino transport or extension works — e.g. `pino-pretty` for friendlier dev output.
...
const client = new Client({
    plugins: [
        logger({
            name: 'slipher-bot', // becomes evlog env.service
            transports: [
                evlogTransport({
                    env: {
...
Any evlog drain works — Axiom, OTLP, Sentry, fs, or your own pipeline. If you initialize evlog yourself, call `evlogTransport()` or `evlogRenderer()` without config; Slipher then leaves the global setup and `silent` choice to you.
