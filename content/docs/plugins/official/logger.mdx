...
<Callout type="warn">
**Redaction belongs to the sink.** `prettyRenderer()` does **not** redact. Configure redaction in your runtime or collector, in your Pino instance, or in evlog's config. evlog's built-in patterns (`creditCard`, `email`, `jwt`, …) do **not** cover Discord bot tokens — add a pattern for those.
</Callout>
