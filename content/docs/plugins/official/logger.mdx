...
```text
Seyfert interaction lifecycle
  ├── command observers ───────────────┐
  ├── component/modal transforms ─────┤
  └── context scope (AsyncLocalStorage)┤
                                      ▼
                         ctx.logger / useLogger()
                              WideEventLogger
                                      │
                     automatic outcome + duration
                                      ▼
                                 RootLogger
                        ┌─────────────┴─────────────┐
                        ▼                           ▼
                 one renderer                 N transports
          pretty / silent / Pino / evlog    Pino / evlog / custom
                        └─────────────┬─────────────┘
                                      ▼
                           terminal / log backend
```
