...
class AudioProcessor {
	...
	onCompleted({ job, jobId, result }: { job: QueueJobOf<'audio'> | undefined; jobId: string; result: string }) {
		job?.snapshot();
		void jobId;
		void result;
	}
...
Decorated `@Process()` handlers receive the active Seyfert client as their second argument. Processor classes are still resolved eagerly, but the plugin waits until setup has bound the client before attaching them to the queue. With `memory()`, jobs queued during startup remain waiting. With `persistent()`, existing Redis jobs are not consumed before setup, while new `add()` calls reject until the driver is active.

Registries created directly with `createQueues()` stay framework-independent. Their processors receive `undefined` unless you pass a compatible client to `registry.setup(client)`. The imperative `queue.process(job => ...)` API remains driver-level and receives only the job; capture the client from the surrounding scope when needed:
...
`@OnQueueEvent(event)` is the queue/global channel and accepts `added`, `completed`, and `failed`. With `memory()` both decorators observe the same single-process queue. With `persistent()`, queue-level events come from BullMQ `QueueEvents` and use one extra Redis connection per queue per replica.

All listeners receive one object payload: `{ job }`, `{ job, result }`, `{ job, error }`, `{ job, error, delay }`, or `{}` for `idle`. Global payloads always include `jobId`. Persistent queue-wide payloads currently leave `job` undefined; use worker events when you need the hydrated job in the process that handled it, or use `jobId` as the stable identity and call `queue.getJob(jobId)` while BullMQ still retains the job. Direct queue instances also support `on()` and `once()`.
...
```mermaid
flowchart TD
    accTitle: Queues plugin architecture
    accDescr: A command, event, or service produces typed jobs through the shared queues registry. The selected memory or persistent BullMQ driver delivers those jobs to processor handlers and event listeners.

    producer["Producer<br/>(command / event / service)"]
    access["ctx.queues / client.queues / plugin.registry"]
    registry["QueuesRegistry<br/>typed Queue&lt;TData, TResult&gt;"]
    memory["memory() driver<br/>in-process queue"]
    persistent["persistent() driver<br/>BullMQ Queue + Worker + QueueEvents"]
    handlers["@Processor + @Process handler<br/>@OnWorkerEvent / @OnQueueEvent listeners"]
...
The plugin creates one registry and exposes that same object through `ctx`, the client, and the plugin instance. During `setup`, the selected driver opens or activates its queues and workers; `teardown` closes the registry and every driver resource. The driver boundary keeps producer and processor code unchanged when moving from process-local memory to Redis-backed BullMQ.
...
Set `autostart: false` on either driver when a processor should be registered without consuming jobs until `await queue.start()`. Use `await queue.pause()` to pause consumption explicitly.

Queues created after the Seyfert client starts initialize lazily on their first asynchronous operation. Await late processor registration so connection and worker-readiness errors remain observable:
