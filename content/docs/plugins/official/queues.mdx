...
All listeners receive one object payload: `{ job }`, `{ job, result }`, `{ job, error }`, `{ job, error, delay }`, or `{}` for `idle`. Global payloads always include `jobId`. With `persistent()`, `job` can be `undefined` when BullMQ has already removed it through `removeOnComplete` or `removeOnFail`; use `jobId` as the stable identity and call `queue.getJob(jobId)` when BullMQ still retains the job. Direct queue instances also support `on()` and `once()`.
