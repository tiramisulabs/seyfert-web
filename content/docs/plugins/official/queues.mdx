...
`@OnWorkerEvent(event)` is local to the process that ran the job — use it for local cache updates or process-local instrumentation. `@OnQueueEvent(event)` is the queue/global channel and accepts `added`, `completed`, and `failed`. With `memory()` both decorators observe the same single-process queue. With `persistent()`, queue-level events come from BullMQ `QueueEvents` and use one extra Redis connection per queue per replica.

All listeners receive one object payload: `{ job }`, `{ job, result }`, `{ job, error }`, `{ job, error, delay }`, or `{}` for `idle`. Global payloads always include `jobId`. With `persistent()`, BullMQ `QueueEvents` payloads use `job: undefined`; use `jobId` as the stable identity and call `queue.getJob(jobId)` when you need a job that BullMQ still retains. Direct queue instances also support `on()` and `once()`.
...
await lateQueue.add({ fileId: 'file-1' });
