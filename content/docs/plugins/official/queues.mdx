...
```text
producer (command / event / service)
                 │
                 ▼
 ctx.queues / client.queues / plugin.registry
                 │
                 ▼
           QueuesRegistry
        typed Queue<TData, TResult>
                 │
        ┌────────┴─────────┐
        ▼                  ▼
 memory() driver     persistent() driver
 in-process queue    BullMQ Queue + Worker + QueueEvents
        │                  │
        └────────┬─────────┘
                 ▼
 @Processor + @Process handler
 @OnWorkerEvent / @OnQueueEvent listeners
```
