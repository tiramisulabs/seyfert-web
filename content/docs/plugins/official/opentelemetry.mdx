...
```text
Seyfert runtime surfaces
  ├── interactions ── context scope + lifecycle observers ──┐
  ├── gateway events ───────── event wrapper ───────────────┤
  ├── Discord REST ─────────── api.rest observer ───────────┤
  └── cache adapter ────────── method wrappers ─────────────┤
                                                            ▼
                                            @slipher/opentelemetry
                                            spans + duration metrics
                                                            │
                                      global OpenTelemetry API provider
                                               ┌────────────┴────────────┐
                                               ▼                         ▼
                                      plugin-owned NodeSDK        host/preloaded SDK
                                               └────────────┬────────────┘
                                                            ▼
                                                exporter / collector
```
