...
Cron timezone follows the selected driver: `memory()` delegates to Croner's runtime timezone, while `persistent()` delegates repeated cron scheduling to BullMQ. If timezone matters, run workers with an explicit process timezone such as `TZ=UTC` and write cron expressions for it.
...
Supported events: `scheduled`, `started`, `completed`, `failed`, `paused`, `resumed`, `removed`, and `error`. Persistent BullMQ resources emit `error` with `{ source, error }`, where `source` is `queue`, `queue-events`, or `worker`; transport errors are also sent to the configured logger.
