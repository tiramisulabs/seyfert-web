...
```text
@Cron / @Interval classes        registry.cron / interval / add
             │                                  │
             └──────────────┬───────────────────┘
                            ▼
                     SchedulerRegistry
             ctx.scheduler = client.scheduler
                            │
                 ┌──────────┴──────────┐
                 ▼                     ▼
          memory() driver       persistent() driver
             Croner          BullMQ Queue + Worker
                 │             + QueueEvents + Redis
                 └──────────┬──────────┘
                            ▼
                  task runner + lifecycle events

Seyfert setup ──> prepare resources
plugins:ready ──> activate tasks
client.close ──> close local driver resources
```
