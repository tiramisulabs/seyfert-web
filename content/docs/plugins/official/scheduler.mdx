...
The persistent driver supports BullMQ 5.23+ and 6.x.
...
Once registered, the plugin exposes `ctx.scheduler` and `client.scheduler`. The Seyfert lifecycle handles setup for you when the client starts, and teardown when it closes. Task callbacks receive the scheduled task and the active Seyfert client, so task modules do not need to import the client from the application entrypoint.
...
ctx.scheduler.cron(
	...
	'0 0 * * *',
	async (_task, client) => {
		client.logger.info('Running daily cleanup');
		// run work
	},
	{ timezone: 'America/Santo_Domingo' },
);
...
The default BullMQ queue name is `slipher-scheduler`. You can also pass an existing ioredis connection. BullMQ duplicates it for blocking Worker and `QueueEvents` connections, so configure `maxRetriesPerRequest: null` and close the shared connection only after the scheduler:
...
const registry = createScheduler({ driver: persistent({ connection }) });
...
registry.cron('daily-cleanup', '0 0 * * *', async (_task, client) => {
	void client;
	// run work
});

// add() resolves cron vs. interval from the value automatically
registry.add('poller', '10s', async (task, client) => {
	void task.runCount;
	void client;
});
...
Standalone registries keep the client optional. The second callback argument is `undefined` after `registry.setup()`, or the exact compatible client passed to `registry.setup(client)`.
