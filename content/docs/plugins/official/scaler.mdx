...
This guide documents an unreleased feature that is still under active development. Its API and configuration may change before Seyfert v5 ships on npm, so do not use it in production yet.
</Callout>
...
<Callout type="info">
Requires a generation-aware Seyfert v5 build. The scaler uses `WorkerManager` custom mode and its generation lifecycle; it is infrastructure you construct directly, not a `SeyfertPlugin` installed on `Client`.
</Callout>
