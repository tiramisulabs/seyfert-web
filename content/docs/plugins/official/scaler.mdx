...
description: Discover hosts, place gateway workers, and redistribute them live with @slipher/scaler.
---
...
Install the scaler beside Seyfert on the master and every agent host:
...
With `generationLifecycle: 'eager'`, the candidate loads the application while shadowed. This gives the strongest readiness check, but startup hooks must tolerate overlapping generations. Use `'deferred'` when duplicate startup side effects are more dangerous than discovering an application startup failure after the source drains.
...
`createLogicalWorkers()` accepts an exclusive `shardEnd`, matching `WorkerManager`. Each generated logical worker stores an inclusive `shardEnd` for placement.
