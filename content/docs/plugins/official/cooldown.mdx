...
Tiered limits such as premium users receiving more uses are intentionally outside the 1.0 surface. Keep that policy in application code rather than passing a resolver where `uses` expects a number.
...
Commands that share a `group` share the same cache key. Namespace and target segments are encoded, so spaces and separators in custom values cannot collide: `${encodeURIComponent(group ?? resolvedCommandName)}:${typeLabel}:${encodeURIComponent(target)}`.
...
Commands in one group should use the same `type`, `interval`, and `uses`. The command being consumed supplies those settings, so mixing policies makes the effective limit depend on call order.

## Storage atomicity

`consume` is atomic across processes when the cache adapter explicitly opts in to `AtomicCooldownAdapter` with `supportsAtomicCooldowns: true` and a Redis-compatible `eval(script, keys, args)` method. Adapters that merely expose `eval` without that marker use the regular read-decide-write path.
...
For non-atomic async adapters, one manager serializes `consume` and `reset` per bucket key. That prevents overspend inside one process without blocking unrelated buckets, but it cannot coordinate separate processes or replicas.

<Callout type="info">
`check` is a read-only preview and can become stale immediately. Use `consume` as the admission authority, especially in multi-worker bots.
</Callout>
