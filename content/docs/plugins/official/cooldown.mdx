...
```text
Seyfert command invocation
              │
              ├── @Cooldown metadata ───────────────┐
              │                                     │
              └── plugin context scope              ▼
                  + optional middleware ───> CooldownManager
                                                   │
                                      check / consume / reset
                                                   │
                                                   ▼
                                    client.cache / CooldownResource
                                                   │
                              ┌────────────────────┴────────────────────┐
                              ▼                                         ▼
                     regular cache adapter               AtomicCooldownAdapter
                     (process serialization)              (Redis EVAL + Redis TIME)
```
