...
During 0.x, prefer direct supported entry points over subclassing exported classes: build bot tests through `createMockBot()`, seed worlds through `mockWorld()`, and use named dispatchers such as `slash()`, `clickButton()`, and `emit()`. Unspecified and subject to change during 0.x: `mockId()` format, warning text, `RecordedAction.seq`, and `MockGateway`.
