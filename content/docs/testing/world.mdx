...
You build a world with `mockWorld()`, register entities onto it, and pass it to `createMockBot({ world })`. During the test you assert on it through the views on `bot` (`bot.worldGuild`, `bot.worldChannel`, `bot.worldDm`, …) and, for the long tail of entity types, the read-only `bot.world` reader.
...
The guild/channel/role/DM views are the primary assertion surface:
