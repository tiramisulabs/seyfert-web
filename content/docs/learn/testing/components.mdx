...
This builds on [Setup](/docs/learn/testing/setup). You already have a mock bot running your real pipeline. Now you will click buttons and pick select-menu values, and watch your component handlers reply for real.
...
<Callout>
For a `ComponentCommand` select-menu path that has no collector, build the raw payload with `selectMenuInteraction()` and feed it to `dispatchInteraction()`. See [The Toolkit › Dispatching](/docs/testing/dispatching).
</Callout>
...
- Next: [Modals](/docs/learn/testing/modals) for the button → modal → submit flow.
- Reference: [The Toolkit › Dispatching](/docs/testing/dispatching) for every dispatch helper.
