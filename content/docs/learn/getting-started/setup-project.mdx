...
<Callout type="info" title="Recommended: type-aware linting">
Now that TypeScript is set up, consider adding [`@slipher/eslint-plugin`](/docs/recipes/eslint) — a **type-aware** ESLint plugin that reads Seyfert's own types to catch common mistakes as you write. See the [ESLint recipe](/docs/recipes/eslint) to set it up.
</Callout>
...
<Callout>
If you're using Cloudflare Workers, we recommend following the guide on their [official page](/docs/recipes/cloudflare-workers).
</Callout>
