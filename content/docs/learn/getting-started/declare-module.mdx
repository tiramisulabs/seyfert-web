...
declare module 'seyfert' {
    interface SeyfertRegistry {
        client: ParseClient<Client<true>>; // Gateway [!code highlight]
...
declare module 'seyfert' {
    interface SeyfertRegistry { middlewares: typeof allMiddlewares } // [!code highlight]
}
```
...
declare module 'seyfert' {
    interface GlobalMetadata extends ParseGlobalMiddlewares<typeof globalMiddlewares> {} // [!code highlight]
}
```
...
declare module 'seyfert' {
    interface SeyfertRegistry { langs: ParseLocales<typeof defaultLang> } // [!code highlight]
}
```
...
Setting this property to `true` tells Seyfert whether the cache will return a promise. By default, Seyfert uses `MemoryAdapter`, a RAM cache that doesn't return promises, while [`RedisAdapter`](https://npmx.dev/package/@slipher/redis-adapter) does.
...
Read your extended properties at runtime with `client.getRC()`:
