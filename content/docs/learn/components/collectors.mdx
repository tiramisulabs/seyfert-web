...
Collectors are built using the [`createComponentCollector`](/docs/api/BaseMessage:Class#member-Method-createComponentCollector-3) method on a message, which is inherited by [`BaseMessage`](/docs/api/BaseMessage:Class). This method returns an object representing a collector.
