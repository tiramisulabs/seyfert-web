...
`threads` and `clusters` distribute workers inside one machine. When the same gateway topology must run across several VPSs or hosts, [`@slipher/scaler`](/docs/plugins/official/scaler) connects agents to one master, places logical workers by capacity, and redistributes them live when hosts join or leave.
