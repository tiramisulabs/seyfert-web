...
`threads` and `clusters` distribute workers inside one machine. When the same gateway topology must run across several VPSs or hosts, [`@slipher/scaler (WIP)`](/docs/scaling) connects agents to one master and places logical workers by host capacity.

The scaler keeps each logical worker's shard range stable while moving its physical process. It does not automatically rebalance when a host joins, and it is separate from changing the total number of Discord shards.
