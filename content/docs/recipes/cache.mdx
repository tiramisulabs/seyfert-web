...
Use `@slipher/redis-adapter` when multiple processes need to share Seyfert's cache:

```sh
pnpm add @slipher/redis-adapter
```
...
For Redis TTLs and an optional process-local read-through cache, use `ExpirableRedisAdapter`. Resource overrides inherit omitted values from `default`:
...
const adapter = new ExpirableRedisAdapter(
    ...
    {
        default: { expire: 5 * 60_000, ondemand: true, limit: 1_000 },
        message: { expire: 30_000, limit: 100 },
        presence: { ondemand: false },
    },
);
...
| Option | Behavior |
| --- | --- |
| `expire` | Redis TTL in milliseconds. For hash/object writes, positive values refresh it, non-positive values persist the key, and `undefined` preserves the current TTL. Relationship members are separate string keys: positive values set their TTL; otherwise an add stores them without a TTL. |
...
| `native` | Disables the adapter-local cache for a resource whose supplied node-redis client already uses RESP3 `clientSideCache`. It does not enable native caching itself. |

The local cache respects Redis' remaining `PTTL`, but it is process-local and receives no cross-process invalidations. If several processes update the same data, keep `ondemand` disabled or provide a node-redis client configured for RESP3 client-side caching and set `native: true` in the applicable default or resource policy.

Both adapters own clients they construct. Close that client during shutdown:
