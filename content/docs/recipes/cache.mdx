...
```ts
<resource>.<id2>.<id1> // member.1003825077969764412.366779196975874049
<resource>.<id1>       // user.576805413148688424
```
...
interface SeyfertDotDev {
    // Commits the value and its relationship as one logical operation.
    set(key: string, value: any, relationship: readonly [to: string, id: string]): Promise<void>;
    patch(key: string, value: any, relationship: readonly [to: string, id: string]): Promise<void>;
...
// @noErrors
// ---cut---
import { Adapter, type AdapterEntry, type AdapterRelationship } from 'seyfert';
...
class MyAdapter implements Adapter {
    ...
    // --- Storing values ---
    async set(key: string, value: any | any[], relationship: AdapterRelationship) {
        await this.storage.set(key, value, relationship);
    }

    async bulkSet(entries: AdapterEntry[]) {
        for (const entry of entries) await this.set(...entry);
    }

    // patch only overwrites the properties you pass, not the whole value
    async patch(key: string, value: any | any[], relationship: AdapterRelationship) {
        await this.storage.patch(key, value, relationship);
    }

    async bulkPatch(entries: AdapterEntry[]) {
        for (const entry of entries) await this.patch(...entry);
    }
    ...
    // --- Reading/removing relationships (sets of unique IDs) ---
...
```ts
<resource>.<*>.<*> // member.*.*
<resource>.<*>.<id> // member.*.366779196975874049
