...
// Roles
createEvent({ data: { name: 'guildRoleCreate' }, run(role, client) {} });
createEvent({ data: { name: 'guildRoleUpdate' }, run([newRole, oldRole], client) {} });
createEvent({ data: { name: 'guildRoleDelete' }, run(role, client) {} });
...
// Set presence
import { PresenceUpdateStatus, ActivityType } from 'seyfert';
