...
This guide continues from [Distributed scaling](/docs/scaling/distributed) and covers the operational behavior of `@slipher/scaler`.

<Callout type="warn" title="Work in progress">
`@slipher/scaler` is not published on npm yet. This guide tracks [its current pull request](https://github.com/tiramisulabs/extra/pull/27), so its API can still change before release.
</Callout>
