...
sequenceDiagram
    ...
    accDescr: A WorkerClient sends every cache operation over process or thread IPC to WorkerManager. The manager executes the operation on its configured cache backend and returns the result over IPC. The worker keeps a pending promise until the response arrives or its 60-second timeout expires.
    ...
    Manager-->>Worker: CACHE_RESULT(nonce, result)
```
...
In the current protocol, the manager returns only successful `CACHE_RESULT` messages. If its backend throws before sending the result, the worker receives no error payload and the pending request rejects with `CACHE_TIMEOUT` after 60 seconds.

### Recommended: connect each worker to Redis

Give every worker a Redis adapter with the same namespace. Workers then share one external source of truth without routing cache traffic through `WorkerManager`:
