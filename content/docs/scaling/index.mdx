...
```text
operator / deployment
        │ start, assign, handoff, reconcile, stop
        ▼
master process
┌───────────────────────────────────────────────────────────────┐
│ SeyfertScaler                                                 │
│ topology + placement + allocation lifecycle                   │
│        │ launch / stop / route                                │
│        ▼                                                      │
│ ScalerMaster ── global Discord IDENTIFY bucket gate           │
└───────────────┬───────────────────────────────────────────────┘
                │ authenticated JSON over WebSocket
       ┌────────┴─────────┐
       ▼                  ▼
agent host A          agent host B
┌──────────────┐      ┌──────────────┐
│ ScalerAgent  │      │ ScalerAgent  │
│ process      │      │ process      │
│ supervision  │      │ supervision  │
└──────┬───────┘      └──────┬───────┘
       │ ProcessWorkerRunner  │ ProcessWorkerRunner
       ▼                      ▼
 vanilla WorkerClient      vanilla WorkerClient
 fixed shard range         fixed shard range
       └──────────────┬───────┘
                      ▼
               Discord Gateway

worker: WORKER_START → SPAWN_SHARDS
shard:  CONNECT_QUEUE → IDENTIFY gate → ALLOW_CONNECT
ready:  WORKER_READY → routed
health: HEARTBEAT → ACK_HEARTBEAT
```
...
```text
startup: unassigned → launching → routed
handoff: routed → stopping → unassigned → launching(target) → routed
failure: routed → unassigned + downtime
```
